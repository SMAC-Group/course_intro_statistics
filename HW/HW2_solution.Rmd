...
Based on these graphs, it is reasonable to believe that some outliers are present. Thus, we select Wilcoxon test to compare the mean in different groups. 
```{r}
wilcox.test(Cmax_women, Cmax_men, alternative = "less")
wilcox.test(Cmax_obese, Cmax_non_obese, alternative = "less")
```

From the result we can see that the p-value = 0.001661 in the first test is smaller than 0.05, which indicates there are some evidence against the null hypotheses in favor of the alternative. Therefore, at the 95% confidence level, we reject the null and accept the alternative hypothesis that the mean of the variable `cmax` in group of `women` is less than that in `men`. However, for the second test, there isn't an evidence to reject the null hypotheses in which we assert that there is no obvious difference in `cmax` between group of `obese` and `non_obese`.
