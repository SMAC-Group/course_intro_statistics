...
ggplot(counts_penguins) +
  aes(x = reorder(species, count), y = count, fill = species) +
  geom_bar(stat = "identity", width = .7) +
  coord_flip(ylim = c(0, 200)) +
  theme_minimal() +
  xlab("") +
  ylab("")+
  geom_text(stat='identity', aes(label = count), hjust=1.4, col ="white", size = 5)+
  theme(
    ...
    axis.text.y = element_text( color="black", 
                                size=14)
  ) + scale_fill_manual(values = c("Adelie" = "#3d6721", 
                                   "Gentoo" = "#006c89", 
                                   "Chinstrap" = "#a86826")) +
  annotation_custom(img_adelie, ymin = 153, ymax = 172, xmin = 2.5, xmax = 3.5) +
  annotation_custom(img_gentoo, ymin = 128, ymax = 147, xmin = 1.5, xmax = 2.5) +
  annotation_custom(img_chinestrap, ymin = 78, ymax = 97, xmin = 0.5, xmax = 1.5) +
